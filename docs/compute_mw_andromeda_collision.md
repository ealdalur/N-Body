...
2. **Singular isothermal sphere / no truncation radius**: The enclosed mass formula M = v_c^2 * r assumes an SIS profile extending to infinity. The simulation uses a cored isothermal profile (M ~ v_c^2 * r only for r >> r_c), which reduces the enclosed mass at small radii. At r=2500-3000 >> r_c=167-200, the formula is a good match to what the code does.

   What neither the formula nor the code captures is that a real halo **stops** at roughly its virial radius. Here M_halo grows linearly with r forever (which is exactly what a flat rotation curve demands: v^2 = GM/r = const implies M ~ r). At the initial 3000-unit separation the two halos supply ~3e8 in code mass against ~1.6e7 of baryons, so the encounter is ~95% halo-driven — that fraction is roughly right and is not itself the error. The error is that some of that mass should not be there: 3000 code units is ~180 kpc, near the real virial radius, where the SIS overshoots the MW's enclosed mass by ~1.5x. Long-range attraction is therefore somewhat overestimated and infall somewhat too fast, and the overshoot would grow linearly if the galaxies were started farther apart. Adding a truncation radius (capping M_halo beyond ~r_200 so the field falls back to 1/r^2) would be the cheapest single improvement to this script's realism.

3. **No dynamical friction, anywhere**: As the galaxies approach, dynamical friction from overlapping halos should decelerate them and drive the merger. Neither the setup script nor the simulation models this. The cross-halo gravitational terms do **not** supply it: they are conservative central forces between two rigid spherical potentials, each comoving with its own galaxy's barycenter, and by symmetry such a configuration exerts zero net drag. Only the live baryonic particles produce friction, and here they are ~5% of the gravitating mass at the initial separation. This is the single largest source of error in this script — **the merger will proceed far more slowly than the real MW–Andromeda encounter, with too many wide passages before coalescence, if it coalesces at all.** Treat the first passage as meaningful and everything after it as qualitative.
