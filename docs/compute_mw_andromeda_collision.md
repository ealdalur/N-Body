...
2. **Singular isothermal sphere, halo left untruncated**: The enclosed mass formula M = v_c^2 * r assumes an SIS profile extending to infinity. The simulation uses a cored isothermal profile (M ~ v_c^2 * r only for r >> r_c), which reduces the enclosed mass at small radii. At r=2500-3000 >> r_c=167-200, the formula is a good match to what the code does.

   A real halo **stops** at roughly its virial radius, whereas an untruncated SIS grows linearly with r forever (which is what a flat rotation curve demands: v^2 = GM/r = const implies M ~ r). The simulator **does** support truncation — `halo_truncation_radius = Rh` freezes M_halo beyond Rh and reverts the field to 1/r^2 — but this collision script leaves `Rh = 0` (untruncated), whereas the M51 scripts set `Rh = Rd`. At the initial 3000-unit separation the two halos supply ~3e8 in code mass against ~1.6e7 of baryons, so the encounter is ~95% halo-driven — that fraction is roughly right and is not itself the error. The error is that some of that mass should not be there: 3000 code units is ~180 kpc, near the real virial radius, where the SIS overshoots the MW's enclosed mass by ~1.5x. Long-range attraction is therefore somewhat overestimated and infall somewhat too fast, and the overshoot would grow linearly if the galaxies were started farther apart. Setting `Rh ~ r_200` on this script would remove it.

3. **No halo dynamical friction**: As the galaxies approach, dynamical friction from overlapping halos should decelerate them and drive the merger. The **rigid** analytic halos cannot supply it: a non-deforming spherical potential holds no trailing wake, so it exerts no Chandrasekhar drag regardless of how its centre moves. (The halo centres are integrated as inertial bodies under gravity — see `docs/dark-matter-halo.md` — so the cross-halo interaction is momentum-conserving and the orbit does not decay *spuriously*; it simply lacks the halo-friction channel.) Only the live baryonic particles produce friction, and here they are ~5% of the gravitating mass at the initial separation. This is the single largest source of error in this script — **the merger will proceed far more slowly than the real MW–Andromeda encounter, with too many wide passages before coalescence, if it coalesces at all.** Treat the first passage as meaningful and everything after it as qualitative.

   Note this pushes against the error in limitation 2 above: the untruncated halos overestimate long-range attraction (speeding infall) while the missing friction underestimates orbital decay (slowing the merger). These do not cancel in any controlled way.
